...
function CoachCard({ coach, index }: { coach: CoachData; index: number }) {
  ...
  const displayPhoto = coach.photo ?? '/images/placeholder.jpg'
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col justify-center py-8 lg:py-10 lg:px-10 lg:[direction:ltr]">
        ...
        <motion.h3
          className="font-display font-bold text-3xl md:text-4xl uppercase mb-2"
        ...
        {coach.bio.split('\n\n').map((paragraph, pi) => (
          <motion.p
            key={pi}
            className={`text-apollo-muted text-sm leading-relaxed ${pi > 0 ? 'mt-3' : ''}`}
            initial={{ opacity: 0, y: 20 }}
            animate={isInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }}
